...
- `content: HTMLElement | undefined` - контент модального окна
- `events: IEvents` - экземпляр класса `EventEmitter` для инициации событий при изменении данных.  
...
Представляет собой корзину при начальной загрузке страницы

Свойства класса:
- ` _basket: HTMLButtonElement` - кнопка корзины 
- ` _goods: HTMLSpanElement` - количество товаров в ней

Методы класса:
- `set goods(value: number)` - устанавливает количество товаров
